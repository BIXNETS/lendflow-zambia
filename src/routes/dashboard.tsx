...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track your LendFlow Africa loan applications, commitment payments and 0% interest repayment schedule." },
      { property: "og:title", content: "My dashboard — LendFlow Africa" },
      { property: "og:description", content: "Track applications, commitments and repayments." },
  ...
  return (
    <AppShell user={user} subtitle="Borrower account">
      <div className="rise">
        ...
        <p className="mt-1 text-[color:var(--color-muted)]">All LendFlow loans are 0% interest — you repay only what you borrow.</p>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard label="Commitments paid" value={money(paidCommitment)} tone="sun" />
        ...
        <div className="mt-8 card overflow-hidden">
          ...
          {apps.length === 0 ? (
            <p className="px-6 py-10 text-center text-sm text-[color:var(--color-muted)]">
              No applications yet. Start one from the home page — you'll pay a 10–15% commitment and borrow at 0%.
            </p>
          ) : (
                ...
                <div key={a.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-6 py-4">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-[color:var(--color-muted)]">
                      {a.id} · commitment {money(a.commitment)} ({a.commitmentPct}%) via {a.provider}
                    </div>
                  </div>
